...
const format = (
  str: string,
  initial?: boolean,
  integerPlaces: number = 9,
  decimalPoints: number = 2,
  minimumDecimalPoints: number = 2
) => {
  let decimal: boolean;
  let [whole = '', part = ''] = str.split('.');
  if (initial) {
    decimal =
      str !== '0' && (minimumDecimalPoints > 0 || part.length > 0)
        ? true
        : false;
  } else {
    decimal = str.includes('.');
  }
  whole = whole.replace(/,/g, '').substring(0, 4);
...
> {
  ...
  blur = (propagate = true) => {
    if (
      propagate &&
      this.context.display === (this as any)._reactInternalFiber.key
    ) {
      this.context.blur();
    }
