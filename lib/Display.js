...
const formatArray = (string, initial) => {
  let decimal = string.includes('.');
  let [whole = '', part = ''] = string.split('.');
  decimal = initial && string !== '0' ? true : decimal;
  whole = whole.replace(/,/g, '').substring(0, 9);
...
export default class Display extends Component {
  ...
  constructor(props) {
    ...
    const valueArray = formatArray(value, true);
  ...
  blur = (propagate = true) => {
    ...
    const valueArray = format(this.state.valueArray, true);


    this.setState({
      active: false,
      value: this.value(value),
      valueArray: this.value(valueArray),
      ...
      <TouchableOpacity style={style} onPress={this.focus}>
        <View style={{flexDirection: 'row'}}>
          ...
          >
            ...
            {empty ? (0).toFixed(2): this.state.valueArray[0]}
            {this.state.valueArray[1]? '.' + this.state.valueArray[1]: null}
          </Text>
          <View  style={[cursorStyle, active ? blink : null]}></View>
        </View>
      </TouchableOpacity>
    );
  }
